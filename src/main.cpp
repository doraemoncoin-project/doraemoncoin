...
#if defined(NDEBUG)
# error "Doraemoncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6bc39237e9341610c42a060db6e2810de8064c18ff9eb0a99b1a2b6e6f480466");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Doraemoncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Doraemoncoin Signed Message:\n";
...
{
    // Doraemoncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 800000); // Doraemoncoin: 800k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Doraemoncoin: 1 days
static const int64 nTargetSpacing = 5 * 60; // Doraemoncoin: 5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Doraemoncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doraemoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x92c8ae8f01736f9f0b42c6ba45679930c6f3c47d73146d0340bd179c4ead1952");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1619088279;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085922622;

        if (fTestNet)
        {
            block.nTime    = 1619088196;
            block.nNonce   = 385565507;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x184cce426447dd0536e99df0eb6f4c448d0ea5947b9597e2e1513f305c67a67d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Doraemoncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoraemoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoraemoncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoraemoncoinMiner(CWallet *pwallet)
{
    printf("DoraemoncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("doraemoncoin-miner");
    ...
    try { loop {
        ...
        printf("Running DoraemoncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoraemoncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoraemoncoinMiner, pwallet));
}
